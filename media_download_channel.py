...
# ====== Parameters ======
user_name = 'admin'
user_password = 'Nopassword@1234'
# ====================================================================
# write logs to log files (False/True)
write_logs = True

path_to_media_archive = 'nvr_videos/'
base_path_to_log_file = 'nvr_videos/'
...
<downloadRequest>
    ...
    @classmethod
    def get_video_tracks_info(cls, auth_handler, cam_ip, utc_time_interval, max_videos, channel):
        return cls.get_tracks_info(auth_handler, cam_ip, utc_time_interval, max_videos, channel)
...
class LogPrinter:
    @staticmethod
    def get_all_tracks(_1, _2, utc_time_interval, _3, _4):
...
def download_media(auth_handler, cam_ip, utc_time_interval, content_type, channel):
    tracks = get_all_tracks(auth_handler, cam_ip, utc_time_interval, content_type, channel)
    download_tracks(tracks, auth_handler, cam_ip, content_type, channel)


@logging_wrapper(before=LogPrinter.get_all_tracks)
def get_all_tracks(auth_handler, cam_ip, utc_time_interval, content_type, channel):
    tracks = []
    while True:
        answer = get_tracks_info(auth_handler, cam_ip, utc_time_interval, content_type, channel)
...
def get_tracks_info(auth_handler, cam_ip, utc_time_interval, content_type, channel):
    if content_type == ContentType.VIDEO:
        return CameraSdk.get_video_tracks_info(auth_handler, cam_ip, utc_time_interval, MAX_NUMBER_OF_FILES_IN_ONE_REQUEST, channel)
    else:
        return CameraSdk.get_photo_tracks_info(auth_handler, cam_ip, utc_time_interval, MAX_NUMBER_OF_FILES_IN_ONE_REQUEST, channel)
...
def download_file_with_retry(auth_handler, cam_ip, track, content_type, channel):
    ...
    file_name = get_path_to_video_archive(cam_ip) + '/' + start_time_text + channel + '.' + content_type 
...
def do_work(camera_ip, start_datetime_str, end_datetime_str, use_utc_time, content_type, channel):
    logger = Logger.get_logger()
    try:
        logger.info('Processing cam {}: downloading {}'.format(camera_ip, content_type, channel))
...
Examples:
  ...
  %(prog)s -u 10.145.17.202 2020-04-15 00:30:00 2020-04-15 10:59:59
    ...
    parser.add_argument ("CHANNEL", help = "channel of IP camera")
...
def main():
    ...
    if parameters:
        try:
            camera_ip = '10.101.60.148'
            init(camera_ip)

            start_datetime_str = parameters.START_DATE + ' ' + parameters.START_TIME
            end_datetime_str = parameters.END_DATE + ' ' + parameters.END_TIME
            channel_cam = parameters.CHANNEL
            content_type = ContentType.VIDEO

            do_work(camera_ip, start_datetime_str, end_datetime_str, parameters.utc, content_type, channel_cam)
